    ...
    <body>
        ...
        <header>
			...
			$db = new mysqli('localhost','root','','list');
             $query = $db->query("SELECT * FROM states WHERE country_id='$id' ORDER BY state_name ASC ");
			 $rowCount = $query->num_rows;
				...
				if($rowCount > 0){
					while($row = $query->fetch_assoc())
					{
						...
						echo '<a href="#"><img src="'.$row[IMAGE].'" class="img3"></a>';
						echo '<a href="#"><p class="btn3">' .$row[state_name].'</p></a>'; 
						echo '</div>';
						echo '</div>';
            		}
					echo '</div>';
